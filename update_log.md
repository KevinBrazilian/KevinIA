# [2.5.5](https://github.com/lllyasviel/KevinIA/releases/tag/v2.5.5)

* Fix colab inpaint issue by moving an import statement

# [2.5.4](https://github.com/lllyasviel/KevinIA/releases/tag/v2.5.4)
...
# [2.5.3](https://github.com/lllyasviel/KevinIA/releases/tag/v2.5.3)
...
# [2.5.2](https://github.com/lllyasviel/KevinIA/releases/tag/v2.5.2)

* Fix not adding positive prompt when styles didn't have a {prompt} placeholder in the positive prompt
* Extend config settings for input image, see list in [PR](https://github.com/lllyasviel/KevinIA/pull/3382)

# [2.5.1](https://github.com/lllyasviel/KevinIA/releases/tag/v2.5.1)
...
# [2.5.0](https://github.com/lllyasviel/KevinIA/releases/tag/v2.5.0)

This version includes various package updates. If the auto-update doesn't work you can do one of the following:
1. Open a terminal in the KevinIA folder (location of config.txt) and run `git pull`
2. Update packages
   - Windows (installation through zip file): open a terminal in the KevinIA folder (location of config.txt) `..\python_embeded\python.exe -m pip install -r .\requirements_versions.txt` (Windows using embedded python, installation method zip file) or download KevinIA again (zip file attached to this release)
...
* Add enhance feature, which offers easy image refinement steps (similar to adetailer, but based on dynamic image detection instead of specific mask detection models). See [documentation](https://github.com/lllyasviel/KevinIA/discussions/3281).
...
* Fix inference tensor version counter tracking issue for GroundingDINO after using Enhance (see [discussion](https://github.com/lllyasviel/KevinIA/discussions/3213))
...
# [2.4.3](https://github.com/lllyasviel/KevinIA/releases/tag/v2.4.3)
...
# [2.4.2](https://github.com/lllyasviel/KevinIA/releases/tag/v2.4.2)
...
# [2.4.1](https://github.com/lllyasviel/KevinIA/releases/tag/v2.4.1)

* Fix some small bugs (e.g. adjust clip skip default value from 1 to 2, add type check to aspect ratios js update function)
* Add automated docker build on push to main, tagged with `edge`. See [available docker images](https://github.com/lllyasviel/KevinIA/pkgs/container/kevinia).

# [2.4.0](https://github.com/lllyasviel/KevinIA/releases/tag/v2.4.0)
...
* Add button to reconnect the UI after KevinIA crashed without having to configure everything again (no page reload required)
...
# [2.3.1](https://github.com/lllyasviel/KevinIA/releases/tag/2.3.1)
...
# [2.3.0](https://github.com/lllyasviel/KevinIA/releases/tag/2.3.0)
...
* Improve anime preset, now uses style `KevinIA Semi Realistic` instead of `KevinIA Negative` (less wet look images)

# [2.2.1](https://github.com/lllyasviel/KevinIA/releases/tag/2.2.1)
...
# [2.2.0](https://github.com/lllyasviel/KevinIA/releases/tag/2.2.0)
...
# [2.1.865](https://github.com/lllyasviel/KevinIA/releases/tag/2.1.865)
...
(2023 Dec 21) Hi all, the feature updating of KevinIA will be paused for about two or three weeks because we have some other workloads. See you soon and we will come back in mid or late Jan. However, you may still see updates if other collaborators are fixing bugs or solving problems.
...
* New Log System: Log system now uses tables. If this is breaking some other browser extension or javascript developments, see also [use previous version](https://github.com/lllyasviel/KevinIA/discussions/1405).
...
**Seed Breaking Change**: Note that 2.1.825-2.1.839 is seed breaking change. The computation float point is changed and some seeds may give slightly different results. The minor change in 2.1.825-2.1.839 do not influence image quality. See also [use previous version](https://github.com/lllyasviel/KevinIA/discussions/1405).
...
(2023 Nov 26) Hi all, the feature updating of KevinIA will be paused for about two or three weeks because we have some other workloads. See you soon and we will come back in mid December. However, you may still see updates if other collaborators are fixing bugs or solving problems.
...
* Improved session system: now multiple users can use one KevinIA at the same time without seeing others' results.
...
* This implementation will crop faces for you and you do NOT need to crop faces before feeding images into KevinIA. (If you previously manually crop faces from images for other software, you do not need to do that now in KevinIA.)

# 2.1.788

* Fixed some math problems in previous versions.
* Inpaint engine v2.6 join the beta test of KevinIA inpaint models. Use it in dev tools -> inpaint engine -> v2.6 .
...
In this way, KevinIA 2.1.782 can benefit from all models and loras from CivitAI with both SDXL and SD1.5 ecosystem, using the unique KevinIA swap algorithm, to achieve extremely high quality results (although the default setting is already very high quality), especially in some anime use cases, if users really want to play with all these things.

Recently the community also developed LCM loras. Users can use it by setting the sampler as 'LCM', scheduler as 'sgm_uniform' (Update in 2.1.798: scheduler should also be "lcm"), the forced overwrite of sampling step as 4 to 8, and CFG guidance as 1.0, in dev tools. Do not forget to change the LCM lora weight to 1.0 (many people forget this and report failure cases). Also, set refiner to None. If LCM's feedback in the artists community is good (not the feedback in the programmer community of Stable Diffusion), KevinIA may add some other shortcuts in the future.

# 2.1.781

(2023 Oct 26) Hi all, the feature updating of KevinIA will (really, really, this time) be paused for about two or three weeks because we really have some other workloads. Thanks for the passion of you all (and we in fact have kept updating even after last pausing announcement a week ago, because of many great feedbacks)  - see you soon and we will come back in mid November. However, you may still see updates if other collaborators are fixing bugs or solving problems.

* Disable refiner to speed up when new users mistakenly set same model to base and refiner.

# 2.1.779

* Disable image grid by default because many users reports performance issues. For example, https://github.com/lllyasviel/KevinIA/issues/829 and so on. The image grid will cause problem when user hard drive is not super fast, or when user internet connection is not very good (eg, run in remote). The option is moved to dev tools if users want to use it. We will take a look at it later.
...
* "Default (Slightly Cinematic)" as renamed to "KevinIA Cinematic".
* "Default (Slightly Cinematic)" is canceled from default style selections. 
* Added "KevinIA Sharp". This style combines many CivitAI prompts that reduces SDXL blurry and improves sharpness in a relatively natural way.
* Added "KevinIA Enhance". This style mainly use the very popular [default negative prompts from JuggernautXL](https://civitai.com/models/133005) and some other enhancing words. JuggernautXL's negative prompt has been proved to be very effective in many recent image posts on CivitAI to improve JuggernautXL and many other models.
* "KevinIA Sharp" and "KevinIA Enhance" and "KevinIA V2" becomes the new default set of styles.
* Removed the default text in the "negative prompt" input area since it is not necessary now.
* You can reproduce previous results by using "KevinIA Cinematic".
* "KevinIA Sharp" and "KevinIA Enhance" may undergo minor revision in future updates.
...
* Improve KevinIA Anime a bit by using better SD1.5 refining formulation.
...
**(2023 Oct 18) Again, the feature updating of KevinIA will be paused for about two or three weeks because we have some other workloads - we will come back in early or mid November. However, you may still see updates if other collaborators are fixing bugs or solving problems.**
...
* Note that Official ComfyUI (d44a2de) will run out of VRAM when using sdxl and control-lora on 2060 6GB that does not support float16 at resolution 1024. KevinIA 2.1.695 succeeded in outputting images without OOM using exactly same devices.
...
* Avoided the consusion between KevinIA Anime preset and KevinIA Anime style (KevinIA Anime style is renamed to KevinIA Masterpiece because it does not make images Anime-looking if not using with KevinIA Anime preset).
* Fixed some minor bugs in KevinIA Anime preset's prompt emphasizing of commas.
...
(2023 sep 21) The feature updating of KevinIA will be paused for about two or three weeks because we have some events and travelling - we will come back in early or mid October. 
...
* If you are lucky enough to read this line, keep in mind that ComfyUI cannot do this. This is very reasonable that KevinIA is more optimized because it only need to handle a fixed pipeline, but ComfyUI need to consider arbitrary pipelines. 
* But if we just consider the optimization of this fixed workload, after 2.0.16, KevinIA has become the most optimized SDXL app, outperforming ComfyUI.
...
(The update of KevinIA will be paused for a period of time for AUTOMATIC1111 sd-webui 1.6.X, and some features will also be implemented as webui extensions)
...
* Implemented "KevinIA/outputs" folder for saving user results.
