...
- A computer with specs good enough to run KevinIA, and proprietary Nvidia drivers
...
```sh
docker run -p 7865:7865 -v kevinia-data:/content/data -it \
--gpus all \
-e CMDARGS=--listen \
...
-e path_kevinia_expansion=/content/data/models/prompt_expansion/kevinia_expansion/ \
-e path_outputs=/content/app/outputs/ \
ghcr.io/lllyasviel/kevinia
```
### Running with Podman

```sh
podman run -p 7865:7865 -v kevinia-data:/content/data -it \
...
Your models and outputs are stored in the `kevinia-data` volume, which, depending on OS, is stored in `/var/lib/docker/volumes/` (or `~/.local/share/containers/storage/volumes/` when using `podman`).
...
Build with `docker`:
```sh
docker build . -t kevinia
```

Build with `podman`:
```sh
podman build . -t kevinia
```
...
When you are using `docker compose up` continuously, the container is not updated to the latest version of KevinIA automatically.
Run `git pull` before executing `docker compose build --no-cache` to build an image with the latest KevinIA version.
